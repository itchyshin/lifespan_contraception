...
```{r}
# main data
dat0 <- read_csv(here("data", "zoo", "resultsBaSTApost2005oldFormat.csv"), na = c("", "NA"), show_col_types = FALSE)
...
dat_m_surg <- dat %>% filter(is.na(Male_Surgical_Mean) == FALSE, is.na(Male_None_Mean) == FALSE) %>% 
  mutate(M_control_m = Male_None_Mean,
         M_control_sd = sqrt(Male_None_N)*Male_None_SE,
         M_control_n = Male_Surgical_N,
...
dat_m_horm <- dat %>% filter(is.na(Male_Hormonal_Mean) == FALSE, is.na(Male_None_Mean) == FALSE) %>% 
  mutate(M_control_m = Male_None_Mean,
         M_control_sd = sqrt(Male_None_N)*Male_None_SE,
         M_control_n = Male_None_N,
         M_hormonal_m = Male_Hormonal_Mean,
         M_hormonal_sd = sqrt(Male_Hormonal_N)*Male_Hormonal_SE,
         M_hormonal_n = Male_Hormonal_N,
         sex = "male",
         type = "hormonal")
...
dat_f_horm <- dat %>% filter(is.na(Female_Hormonal_Mean) == FALSE, is.na(Female_None_Mean) == FALSE) %>% 
  mutate(F_control_m = Female_None_Mean,
         F_control_sd = sqrt(Female_None_N)*Female_None_SE,
         F_control_n = Female_None_N,
...
rbind(
dat_m_horm[ , c(1, 22, 29:32)], # 1
dat_m_surg[ , c(1, 22, 29:32)], # 2
dat_f_horm[ , c(1, 22, 29:32)], # 3 
dat_f_surg[ , c(1, 22, 29:32)] # 4
) -> dat_all
...
# this does not quite work any longer
...
type_diff <- mod_results(mod_all2, mod = "type", group = "species")

combined <- submerge(type_diff,sex_diff, main)
...
# comb2
sex_type_diff <- mod_results(mod_all3, mod = "sex_type", group = "species")

combined <- submerge(sex_type_diff, main)
...
# same combined as rds 

# combined
saveRDS(combined, file = here("Rdata", "fig", "fig1_comb1.rds"))

# sex_type
saveRDS(combined, file = here("Rdata", "fig", "fig1_comb2.rds"))

# sex + sex_type
# sex_type
saveRDS(combined, file = here("Rdata", "fig", "fig1_comb3.rds"))



sex_type_talbe <- sex_type_diff$mod_table[c(1,4), ]

sex_type_dat <- sex_type_diff$data[sex_type_diff$data$moderator == "Female_hormonal" | sex_type_diff$data$moderator == "Male_surgical", ]

sex_type_diff2 <- list(mod_table = sex_type_talbe, data = sex_type_dat)

class(sex_type_diff2) <-  c("orchard", "data.frame") 


combined <- submerge(sex_type_diff2,sex_diff, main)
...
combo <- orchard_plot(combined,
  ...
  scale_colour_manual(values = rev(c("#999999", "#88CCEE", "#CC6677", "#117733",  "#332288"))) +
  scale_fill_manual(values = rev(c("#999999", "#88CCEE", "#CC6677", "#117733",  "#332288")))
