...
write.tree(tree, file=here("data", "literature", "tree_rotl.tre")) #save the tree
...
# alternative function from metafor package
#V_matrix <- vcalc(vi = vi, 
#             cluster = Shared_control, 
#             obs = Effect_ID, 
#             data = dat, 
#             rho = 0.5) # the same result
...
```{r}
# remove odl data
dat_full <- read.csv(here("data", "healthspan", "healthspan4.csv"), na = c("", "NA"))
...
# note that directories may not match
# also the code may not run if not matching to the section (we have 5 sections here)
...
## Example of survival analysis on contraception and cause of death cumulative incidence
...
CalcCumInc <- function(x, SxLevs = NULL) {
  ...
  PLEcause <- list()
  for (ic in 1:nCause) {
    idc <- which(cdDat$Cause == causes[ic])
...
# creating mod_table- results from the model (Section I)
# mod is from the literature lifespan meta-aanalysis
