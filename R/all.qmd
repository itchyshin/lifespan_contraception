...
## Meta-regression: primates or not

```{r}

# create a variable which sort out primates or not (order)
dat_all$primates_or_not <- ifelse(dat_all$order == "Primates", "primates", "not primates")

mod_all0 <- rma.mv(yi, V = VCV,
                   mod = ~ primates_or_not - 1,
                   random = list(
                     ~1|species,
                     ~1|phylogeny,
                     ~1|obs_id),
...
orchard_plot(mod_all0, mod = "primates_or_not",
...
## Meta-regression: sex x primates or not


```{r}

dat_all$sex_primates_or_not <- paste(dat_all$sex, dat_all$primates_or_not, sep = "_")

mod_not_all1 <- rma.mv(yi, V = VCV,
                   mod = ~ sex_primates_or_not - 1,
                  random = list(
                    ~1|species,
                    ~1|phylogeny,
                    ~1|obs_id),
                  ...
                  test = "t",
                  sparse = TRUE)
summary(mod_not_all1)
round(r2_ml(mod_not_all1), 5)

orchard_plot(mod_not_all1, mod = "sex_primates_or_not",
...
## Meta-regression: type x primates or not

```{r}

dat_all$type_primates_or_not <- paste(dat_all$type, dat_all$primates_or_not, sep = "_")

mod_not_all2 <- rma.mv(yi, V = VCV,
                 mod = ~ type_primates_or_not -1,
                 random = list(
                   ~1|species,
                   ~1|phylogeny,
                   ~1|obs_id),
...
orchard_plot(mod_not_all2, mod = "type_primates_or_not",
             xlab = "lnRR (all)", group = "species", g = FALSE, angle = 45)
...
d# PART IV: ANALYSIS OF CAUSES OF DEATH ON ZOO DATA
