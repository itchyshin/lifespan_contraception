...
## Meta-regression: primates vs carnivora

```{r}
# filter out anything other than primates and cornivores
dat_pc <- dat_all %>% filter(order == "Primates" | order == "Carnivora")

VCVpc <- vcalc(vi = dat_pc$vi, 
             cluster = dat_pc$species, 
             obs = dat_pc$obs_id,
             rho = 0.5, 
             data = dat_pc)

mod_all0 <- rma.mv(yi, V = VCVpc,
                   mod = ~ order - 1,
                   random = list(
                     ~1|species,
                     ~1|phylogeny,
...
summary(mod_all0)
round(r2_ml(mod_all0), 5)

orchard_plot(mod_all0, mod = "order",
...
## Meta-regression: sex x primates vs carnivora 

```{r}

dat_pc$sex_order <- paste(dat_pc$sex, dat_pc$order, sep = "_")

mod_not_pc <- rma.mv(yi, V = VCVpc,
                   mod = ~ sex_order - 1,
                  random = list(
                    ~1|species,
                    ~1|phylogeny,
                  ...
                  sparse = TRUE)
summary(mod_not_pc)
round(r2_ml(mod_not_pc), 5)

orchard_plot(mod_not_pc, mod = "sex_order",
...
## Meta-regression: type x primates vs carnivora

```{r}

dat_pc$type_order <- paste(dat_pc$type, dat_pc$order, sep = "_")

mod_not_all2 <- rma.mv(yi, V = VCVpc,
                 mod = ~ type_order -1,
                 random = list(
                   ~1|species,
                   ~1|phylogeny,
...
summary(mod_not_all2)

# R^2 - hardly any
round(r2_ml(mod_not_all2)*100, 5)

orchard_plot(mod_not_all2, mod = "type_order",
             xlab = "lnRR (all)", group = "species", g = FALSE, angle = 45)


# result table
#all_models(mod_all2, mod = "type_primates_or_not")
```
